...
#用opencv来做,图片尺寸不受限制
path1  = r"testimg_star/wang.jpg"
path2 = r"testimg_star/huo.jpg"
image1  = cv2.imread(path1)
image2  = cv2.imread(path2)
_image1 = cv2.cvtColor(image1, cv2.COLOR_BGR2RGB)
_image2 = cv2.cvtColor(image2, cv2.COLOR_BGR2RGB)
boxes1 = detector.detect(_image1)
boxes2 = detector.detect(_image2)
for box in boxes1:
    x1 = int(box[0])
    y1 = int(box[1])
    x2 = int(box[2])
    ...
    X1,output1, cosa1 = net(img, label)
    index = torch.argmax(cosa1, dim=1).item()
for box in boxes2:
    x1 = int(box[0])
    y1 = int(box[1])
    x2 = int(box[2])
    ...
    img = torch.unsqueeze(img, dim=0).to(device)
    label = torch.arange(1).cuda()
    X2,output2, cosa2 = net(img, label)
    index = torch.argmax(cosa2, dim=1).item()
print(X1.shape,X2.shape)
X1 = F.normalize(X1)  # x/|x|
X2 = F.normalize(X2)  # w/|w|
cosa = F.linear(X1,X2)
print(cosa)

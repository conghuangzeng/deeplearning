...
class Detector:
    ...
    def detect_rnet(self,image,p_boxes):
        ...
        # print(cond)
        indexs,_ = np.where(cond>0.6)#由两个数组组成的元组结构
        #建议框的位置
    ...
    def offset_to_boxes(self,indexs,__cond,__offset,scale,side_len = 12,stride=2):
        #建议框的位置
        x11 = np.int16(((indexs[:,1]*stride))/scale)#必须加float，整型除以scale浮点型会有报警
        y11 = np.int16(indexs[:,0]*stride/scale)
        x22= np.int16((indexs[:,1]*stride+side_len)/scale)
        y22 = np.int16((indexs[:,0]*stride+side_len)/scale)
        ow = x22-x11
        oh = y22-y11
        # print(oh,ow)#两个值有些不完全相等，差了一个像素
        #真实框的位置
        ...
        boxes  = np.array((x1,y1,x2,y2,__cond)).T
        # print(type(box))
        # print(box.shape)
        return boxes
